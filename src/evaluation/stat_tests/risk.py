...
DEFAULT_MODEL_PATHS: Dict[str, Path] = {
    "mlp": Path("experiments/risk/runs/mlp_risk_artifacts/predictions.csv"),
    "xgb": Path("experiments/risk/runs/xgb_risk_artifacts/predictions.csv"),
}
DEFAULT_KEY_COLS = ("ticker", "fiscal_year", "period_end", "timeframe")
DEFAULT_OUT_DIR = Path("experiments/risk/runs/stat_tests_validation_artifacts")
BASELINE_NAME = "current_drawdown_baseline"
...
def _clip_unit(arr: np.ndarray) -> np.ndarray:
    return np.clip(np.asarray(arr, dtype=np.float64).reshape(-1), a_min=0.0, a_max=1.0)
...
def _safe_corr(method: str, a: np.ndarray, b: np.ndarray) -> float:
    ...
    if aa.size < 2 or np.allclose(np.nanstd(aa), 0.0) or np.allclose(np.nanstd(bb), 0.0):
        return float("nan")
    if method == "pearson":
        corr = stats.pearsonr(aa, bb).statistic
    elif method == "spearman":
        corr = stats.spearmanr(aa, bb, nan_policy="omit").statistic
    else:
...
def _metric_functions() -> Dict[str, Callable[[np.ndarray, np.ndarray], float]]:
    return {
        ...
        "mae": _mae,
        "r2": _r2,
        "spearman": lambda y_true, y_pred: _safe_corr("spearman", y_true, y_pred),
        "pearson": lambda y_true, y_pred: _safe_corr("pearson", y_true, y_pred),
        "p90_abs_error": lambda y_true, y_pred: float(
            np.quantile(np.abs(np.asarray(y_pred, dtype=np.float64) - np.asarray(y_true, dtype=np.float64)), 0.90)
        ),
        "p95_abs_error": lambda y_true, y_pred: float(
            np.quantile(np.abs(np.asarray(y_pred, dtype=np.float64) - np.asarray(y_true, dtype=np.float64)), 0.95)
        ),
    }


def _loss_views(y_true: np.ndarray, y_pred: np.ndarray) -> Dict[str, np.ndarray]:
    yt = _clip_unit(y_true)
    yp = _clip_unit(y_pred)
    diff = yp - yt
    return {
...
def _validate_same_targets(base: pd.Series, other: pd.Series, model_name: str) -> None:
    ...
    if base_arr.shape != other_arr.shape or not np.allclose(base_arr, other_arr, rtol=0.0, atol=1e-8, equal_nan=True):
...
def _build_aligned_predictions(
    ...
    if include_baseline:
        ...
        baseline_part = baseline_part.rename(columns={"y_pred_baseline": BASELINE_NAME})
        aligned = aligned.merge(baseline_part, on=list(key_cols), how="inner", validate="one_to_one")
        comparison_models.append(BASELINE_NAME)
...
def _pairwise_significance_table(aligned: pd.DataFrame, model_names: Sequence[str]) -> pd.DataFrame:
    ...
    rows: list[dict[str, object]] = []
    for model_a, model_b in itertools.combinations(model_names, 2):
        for loss_name in ("abs_error", "squared_error", "signed_error"):
...
def _severity_bucket_summary(
    aligned: pd.DataFrame,
    model_names: Sequence[str],
    n_buckets: int,
) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    bucket_df = aligned.copy()
    bucket_df["severity_bucket"] = pd.qcut(bucket_df["y_true"], q=n_buckets, duplicates="drop")
    bucket_df["severity_bucket_label"] = bucket_df["severity_bucket"].astype(str)
    ...
    for model_name in model_names:
        ...
        grouped = model_df.groupby("severity_bucket_label", observed=False)
        for bucket_label, group in grouped:
            summary_rows.append(
                {
                    "model": model_name,
                    "severity_bucket": bucket_label,
                    "rows": int(len(group)),
                    "mean_true": float(np.mean(group["y_true"])),
                    "mean_pred": float(np.mean(group[model_name])),
                    "rmse": float(np.sqrt(np.mean(group["abs_err"] ** 2))),
                    "mae": float(np.mean(group["abs_err"])),
                    "median_abs_error": float(np.median(group["abs_err"])),
                }
            )

        groups = [group["abs_err"].to_numpy(dtype=np.float64) for _, group in grouped if len(group) > 0]
...
def _write_outputs(
    ...
    outputs = {
        ...
        "severity_bucket_summary": out_dir / "severity_bucket_summary.csv",
        "severity_bucket_kruskal": out_dir / "severity_bucket_kruskal.csv",
        "severity_bucket_dunn_holm": out_dir / "severity_bucket_dunn_holm.csv",
    ...
    bucket_summary.to_csv(outputs["severity_bucket_summary"], index=False)
    bucket_kruskal.to_csv(outputs["severity_bucket_kruskal"], index=False)
    bucket_dunn.to_csv(outputs["severity_bucket_dunn_holm"], index=False)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run risk-model statistical tests on aligned prediction artifacts."
    )
    parser.add_argument(
        "--prediction-spec",
        action="append",
    ...
    parser.add_argument(
        ...
        type=str,
        default="mlp",
        help="Loaded model whose y_pred_baseline column should be used as the current-drawdown baseline.",
    )
    parser.add_argument(
        "--exclude-baseline",
        action="store_true",
        help="Disable baseline extraction even if y_pred_baseline is available.",
    )
    parser.add_argument(
        "--severity-buckets",
        type=int,
        default=5,
        help="Number of quantile buckets for the severity robustness check.",
    )
    ...
    bucket_summary, bucket_kruskal, bucket_dunn = _severity_bucket_summary(
        aligned=aligned,
        model_names=model_names,
        n_buckets=int(args.severity_buckets),
    )
    ...
    manifest = {
        ...
        "seed": int(args.seed),
        "severity_buckets": int(args.severity_buckets),
