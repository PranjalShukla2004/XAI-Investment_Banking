...
@dataclass
class PlotRiskSHAPConfig:
    shap_dir: Path = Path("experiments/risk/SHAP")
    max_features: int = 15
        ...
        if shap_values_path and shap_values_path.exists():
            with np.load(shap_values_path, allow_pickle=True) as loaded:
                payload["shap_npz"] = {
                    ...
                    "y_pred_model_output": loaded["y_pred_model_output"],
                    "y_pred_baseline": loaded["y_pred_baseline"],
                }
...
def _plot_metric_comparison(
    ...
    metrics = ["val_rmse", "val_mae", "val_spearman"]
    titles = {
    ...
    fig.suptitle("Risk MLP vs XGBoost Benchmark", fontsize=14)
    _save_figure(fig, out_path, dpi)
    plt.close(fig)
    return out_path


def _plot_abs_error_cdf_comparison(
    payloads: Dict[str, Dict[str, Any]],
    out_path: Path,
    dpi: int,
) -> Path | None:
    ...
    for idx, model_name in enumerate(_ordered_model_names(available)):
        ...
        abs_err = pd.to_numeric(preds["abs_error"], errors="coerce").replace([np.inf, -np.inf], np.nan).dropna()
        if abs_err.empty:
            continue
        vals = np.sort(abs_err.to_numpy(dtype=np.float64))
    ...
    ax.set_ylabel("CDF")
    ax.set_title("Absolute Error CDF by Model")
    ax.grid(alpha=0.2)
    ax.legend()
    _save_figure(fig, out_path, dpi)
    plt.close(fig)
...
def _plot_feature_importance_comparison(
    ...
    for idx, model_name in enumerate(_ordered_model_names(importance_frames)):
        df = importance_frames[model_name]
        aligned = (
            df.set_index("feature").reindex(ordered_features)["mean_abs_shap"].fillna(0.0).to_numpy(dtype=np.float64)
        )
    ...
    ax.set_title("Top SHAP Feature Importance: Risk MLP vs XGBoost")
    ax.grid(axis="y", alpha=0.2)
    ax.legend()
    _save_figure(fig, out_path, dpi)
    plt.close(fig)
...
def _plot_actual_vs_pred(
    ...
    ax.set_xlabel("True drawdown severity")
    ax.set_ylabel("Predicted drawdown severity")
...
def _plot_absolute_error_hist(
    ...
    vals = pd.to_numeric(preds["abs_error"], errors="coerce").replace([np.inf, -np.inf], np.nan).dropna()
    if vals.empty:
        return None

    clip_hi = float(np.quantile(vals, 0.99))
    vals = vals.clip(upper=clip_hi)

    plt = _import_pyplot()
    fig, ax = plt.subplots(figsize=(7.5, 4.5), constrained_layout=True)
    ax.hist(vals, bins=40, color="#d2691e", alpha=0.85)
    ax.set_xlabel("Absolute error (capped at 99th percentile)")
    ax.set_ylabel("Count")
    ax.set_title(f"{model_name}: Validation Absolute Error Distribution")
    ax.grid(axis="y", alpha=0.2)
    _save_figure(fig, out_path, dpi)
    plt.close(fig)
    return out_path
...
def _plot_error_vs_target(
    ...
    y_true = pd.to_numeric(preds["y_true"], errors="coerce").to_numpy(dtype=np.float64)
    abs_err = pd.to_numeric(preds["abs_error"], errors="coerce").to_numpy(dtype=np.float64)
    ax.scatter(y_true, abs_err, alpha=0.3, s=14, color="#2a9d8f", edgecolors="none")
    ax.set_xlabel("True drawdown severity")
    ax.set_ylabel("Absolute error")
    ax.set_title(f"{model_name}: Error vs True Target")
    ax.grid(alpha=0.2)
    _save_figure(fig, out_path, dpi)
    plt.close(fig)
    return out_path
...
def _plot_feature_importance_bar(
    ...
    plt = _import_pyplot()
    fig, ax = plt.subplots(figsize=(9, max(4.5, 0.45 * len(plot_df) + 1.5)), constrained_layout=True)
    ax.barh(plot_df["feature"], plot_df["mean_abs_shap"], color="#1f77b4")
    ax.set_xlabel("Mean |SHAP value|")
    ax.set_ylabel("Feature")
    ax.set_title(f"{model_name}: SHAP Global Importance")
...
def _plot_dependence_plots(
    ...
    for rank, feature in enumerate(selected_features, start=1):
        ...
        ax.set_title(f"{model_name}: Dependence Plot {rank} ({feature})")
        ax.grid(alpha=0.2)
    ...
    for idx, row in top_df.iterrows():
        ...
        for pos in range(1, 6):
            ...
            if pd.isna(feature) or pd.isna(shap_val):
                continue
            contrib_rows.append({"feature": str(feature), "shap": float(shap_val)})
        ...
        fig, ax = plt.subplots(figsize=(8.5, max(4.5, 0.55 * len(plot_df) + 2.0)), constrained_layout=True)
        ...
        ax.set_title(
            ...
            f"base={float(row['base_value']):.3f} | model={float(row['y_pred_model_output']):.3f} | "
            f"pred={float(row['y_pred']):.3f} | true={float(row['y_true']):.3f}"
        )
...
def _plot_model_bundle(
    ...
    plot_path = _plot_actual_vs_pred(preds, model_name, plots_dir / "validation_actual_vs_pred.png", cfg.dpi)
    if plot_path:
        created["files"]["actual_vs_pred"] = str(plot_path)

    plot_path = _plot_absolute_error_hist(preds, model_name, plots_dir / "validation_absolute_error_hist.png", cfg.dpi)
    if plot_path:
        created["files"]["absolute_error_hist"] = str(plot_path)

    plot_path = _plot_error_vs_target(preds, model_name, plots_dir / "validation_error_vs_target.png", cfg.dpi)
    if plot_path:
...
def main() -> None:
    ...
    ap.add_argument("--shap-dir", type=str, default="experiments/risk/SHAP")
    ...
    summary_path = cfg.shap_dir / "risk_shap_summary.json"
    ...
    plot_path = _plot_metric_comparison(payloads, root_plots_dir / "model_metric_comparison.png", cfg.dpi)
    if plot_path:
        manifest["root_plots"]["metric_comparison"] = str(plot_path)

    plot_path = _plot_abs_error_cdf_comparison(payloads, root_plots_dir / "model_absolute_error_cdf.png", cfg.dpi)
    if plot_path:
        manifest["root_plots"]["absolute_error_cdf"] = str(plot_path)

    plot_path = _plot_feature_importance_comparison(
        payloads,
        root_plots_dir / "model_feature_importance_comparison.png",
        cfg.max_features,
        cfg.dpi,
    )
    if plot_path:
    ...
    for model_name, payload in payloads.items():
        manifest["models"][model_name] = _plot_model_bundle(model_name, payload, cfg)

    manifest_path = cfg.shap_dir / "plot_manifest.json"
    manifest_path.write_text(json.dumps(manifest, indent=2, default=_jsonable), encoding="utf-8")

    print("Saved risk SHAP plots:")
